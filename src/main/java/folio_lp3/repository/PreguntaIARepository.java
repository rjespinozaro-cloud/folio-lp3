...
public interface PreguntaIARepository extends JpaRepository<PreguntaIA, Long> {
    ...
    @Query("SELECT p FROM PreguntaIA p WHERE p.consulta.pilar.id = :pilarId AND p.calificacion = 'MALA'")
    List<PreguntaIA> findBadRatingsByPilar(@Param("pilarId") Long pilarId);
    
    @Query("SELECT COUNT(p) FROM PreguntaIA p WHERE p.consulta.pilar.id = :pilarId AND p.calificacion = 'MALA' AND MONTH(p.fechaHora) = :mes AND YEAR(p.fechaHora) = :year")
